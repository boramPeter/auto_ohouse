...
from app.common.base_method.screenshot_func import CaptureClassJenkins
...
from web.BasicSetting.exception_func import jenkins_capture_screenshot
import pytest
...
logger_ios = make_logger_ios("jenkins_prod_ios_log.py")
logger_aos = make_logger_aos("jenkins_prod_aos_log.py")
'''
모도님과 테오님을 위한 간략한 설명
- 바이너리에 있는 키값은             
            if 'case_no' in key:
...
'''
def web_timeout_handler(code_execution, locator):
    try:
    ...
    except Exception: # web timeout (TimeoutError, AssertionError 등)
        raise TimeoutException(f"웹 요소 확인 필요: {str(locator)}")

class JenkinsExceptionHandler:
    ...
    def ios_exceptions_handler(
        ...
        try:
            ...
            while retries < max_retries:
                ScreenRecoderJenkins.start_recording(self,current_function_name + str(retries))
                if step is not None:
                    try:
                        step()
                        ScreenRecoderJenkins.stop_recording(self,current_function_name + str(retries))
                        #ScreenRecoderJenkins.delete_recording(self, current_function_name)
                        break
                    except TimeoutException as e:
                        if any(step in current_function_name for step in [
                            "prod_commerce_platform00003_ios_step",
                            "prod_commerce_platform00003_ios_step1",
                            "prod_commerce_platform00001_ios_step3",
                            "prod_commerce_platform00003_ios_step3",
                            "prod_commerce_platform00005_ios_step3",
                            "prod_commerce_platform00007_ios_step",
                            "prod_commerce_platform00008_ios_step",
                            "prod_commerce_platform00009_ios_step",
                            "prod_commerce_platform00010_ios_step",
                            "prod_commerce_platform00011_ios_step",
                            "prod_search00012_ios_step",
                            "prod_o2o00001_ios_step"
                        ]):
                            if retries == 1:
                                ScreenRecoderJenkins.stop_recording(self,current_function_name+str(retries))

                                # 실패 이미지 캡쳐
                                CaptureClassJenkins.capture_screenshot_put_name(self, current_function_name + str(retries))
            ...
            if back_flow is not None:
                try:
                    ...
                    ScreenRecoderJenkins.stop_recording(self,current_function_name)
                    CaptureClassJenkins.capture_screenshot_put_name(self, current_function_name)
                    if current_function_name not in "precondition":
                        app_restart()()
                    logger_ios.debug(f"{current_function_name}: {e}")
                    TestResult().write_result(f"{case_no}_Result", f'*Fail* ({e})')
    ...
    def aos_exceptions_handler(
        ...
        #
        # try:
        #     ScreenRecoderJenkins.start_recording(self, current_function_name)
        #     if step is not None:
        #         try:
        #             step()
        #             ScreenRecoderJenkins.stop_recording(self, current_function_name)
            ...
            while retries < max_retries:
                ...
                if step is not None:
                    try:
                        ...
                        if any(step in current_function_name for step in [
                            "prod_home00022_aos_step",
                            "prod_my_page00015_aos_step",
                            "prod_o2o00008_aos_step",
                            "prod_o2o00005_aos_step",
                            "prod_o2o00007_aos_step",
                            "prod_my_page00004_aos_step"
                        ]):
                            if retries == 1:
                                ScreenRecoderJenkins.stop_recording(self, current_function_name + str(retries))

                                # 실패 이미지 캡쳐
                                CaptureClassJenkins.capture_screenshot_put_name(self, current_function_name + str(
                                    retries))
                                ...
                                class_count = 5 if current_function_name in "common" else 17
                                if ProviderCommonMethod.count_xml_class(self,
                                                                        'android') < class_count and ProviderCommonMethod.count_img_xml_class(self, 'android') < 11:
                                    TestResult().write_result(f"{current_function_name}_failure_yn", 1)
                                # 커스텀 결과 입력
                                if opt_title_exception2 is not None:
                                    TestResult().write_result(opt_title_exception2, f'{opt_result_exception2}')
                            ...
                            # 실패 이미지 캡쳐
                            CaptureClassJenkins.capture_screenshot_put_name(self,
                                                                            current_function_name + str(retries))
                            # 앱 재시작
                            if current_function_name not in "precondition":
                                app_restart()()  # 로깅
                            logger_aos.debug(f"{current_function_name}: {e}")
    ...
    def web_exceptions_handler(
        ...
        try:
            ...
            # 실패 이미지 캡쳐
            try:
                jenkins_capture_screenshot(page, current_function_name + f"_fail")
