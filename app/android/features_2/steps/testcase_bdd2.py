...
# commservice
from app.android.procedure.comm_service.comm_service_00221 import ChipCarouselCheck
from app.android.procedure.comm_service.comm_service_00224 import ItemCarouselCheck
from app.android.procedure.comm_service.comm_service_00220 import Check220
from app.android.procedure.comm_service.comm_service_00226 import ItemCarouselPdpCheck


class ServerChange:
    @when("앱 삭제")
    def test_precondtion001_aos_app_del_2(self):
        ...
        ExceptionHandler.aos_exceptions_handler_pre_condition(self, current_function_name,
                                                              step=lambda: AppInstaller2.del_app(self,UDID.galaxy_udid_2),
                                                              opt_result_exception="두번째 디바이스에서 앱 삭제실패 테스트종료",
                                                              opt_title_exception2="pre-condition2",
                                                              opt_result_exception2="fail"
                                                              )

    @when('디플로이게이트 진입 후 버전 찾아서 설치')
    def test_precondtion001_aos_step_2(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler_pre_condition(self, current_function_name,
                                                              step=lambda: AppInstaller2.do_install(self),
                                                              opt_result_exception="두번째 디바이스에서 앱 설치실패 테스트종료",
                                                              opt_title_exception2="pre-condition2",
                                                              opt_result_exception2="fail"
                                                              )

    @then('open 버튼 노출확인으로 설치확인')
    def test_precondtion001_aos_result_2(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler_pre_condition(self,
                                                              current_function_name,
                                                              step=lambda: ProviderCommonMethod.assert_equal(self, "Open",
                                                                                                             AppInstaller2.actual_result(
                                                                                                                 self)),
                                                              back_flow=lambda: AppInstaller2.back_flow(self),
                                                              opt_result="두번째 디바이스 앱 설치 완료",
                                                              opt_title2="pre-condition2",
                                                              opt_result2="pass",
                                                              opt_result_assert="두번째 디바이스 앱 설치 완료 (열기 문구는 다름)",
                                                              opt_title_assert2="pre-condition2",
                                                              opt_result_assert2="pass",
                                                              opt_result_exception="두번째 단말에서 앱 설치 실패. 강제종료함",
                                                              opt_title_exception2="pre-condition2",
                                                              opt_result_exception2="fail"
                                                              )
    @given("앱 최초설치")
    ...
    def test_precondtion002_aos_step_2(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler_pre_condition(self, current_function_name,
                                               step=lambda: ServerChangeClass.change_qa(self),
                                               opt_result_exception="두번째 디바이스에서 QA 서버 전환 실패. 테스트 강제종료함",
                                               opt_title_exception2="pre-condition2",
                                               opt_result_exception2="fail"
                                               )

    @then('"카카오톡으로 계속하기", "로그인에 문제가 있으신가요?" 문구 확인')
    def test_precondtion002_aos_result_2(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler_pre_condition(self,
                                               current_function_name,
                                               step=lambda: ProviderCommonMethod.assert_equal(self, ["카카오톡으로 계속하기", "로그인에 문제가 있으신가요?"], ServerChangeClass.actual_result(self)),
                                               opt_result="두번째 디바이스에서 QA 서버 전환 완료",
                                               opt_title2="pre-condition2",
                                               opt_result2="pass",
                                               opt_result_assert="두번째 디바이스에서 QA 서버 전환 완료 (로그인 페이지 문구는 실패함)",
                                               opt_title_assert2="pre-condition2",
                                               opt_result_assert2="pass",
                                               opt_result_exception="두번째 디바이스에서 QA 서버 전환 실패. 테스트 강제종료함",
                                               opt_title_exception2="pre-condition2",
...
class Comm_Service:
    ...
    def test_comm_service00221_aos_st_rt_step1(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self, current_function_name,
                                                step=lambda: ChipCarouselCheck.precondition_suggest_chip(self)
                                                )
    @when('https://store-mobile.qa.dailyhou.se/v1/shopping-home api로 칩 캐러셀 리스트 파싱')
    def test_comm_service00221_aos_st_rt_step2(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self, current_function_name,
                                                step=lambda: ChipCarouselCheck.check_chip_carousel(self)
                                                )
    @then('칩 정상적으로 노출되는지 확인')
    def test_comm_service00221_aos_st_rt_check(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self,
                                                current_function_name,
                                                step=lambda: ProviderCommonMethod.assert_equal(self,
                                                                                               True,
                                                                                               ChipCarouselCheck.actual_result(
                                                                                                   self)),
                                                back_flow=lambda: ChipCarouselCheck.back_flow(self)
                                                )

    @when('https://store-mobile.qa.dailyhou.se/v1/shopping-home api로 아이템 캐러셀에 첫 상품 데이터 파싱')
    def test_comm_service00224_aos_st_rt_step(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self, current_function_name,
                                                step=lambda: ItemCarouselCheck.check_item_carousel(self)
                                                )

    @then('아이템 캐러설중 상품정보가 정상노출되는지 확인')
    def test_comm_service00224_aos_st_rt_check(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self,
                                                current_function_name,
                                                step=lambda: ProviderCommonMethod.assert_equal(self,
                                                                                               True,
                                                                                               ItemCarouselCheck.actual_result(
                                                                                                   self)),
                                                back_flow=lambda: ItemCarouselCheck.back_flow(self)
                                                )
    @then('221, 224번 둘다 pass일경우 pass')
    def test_comm_service00220_aos_st_rt_check(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self,
                                                current_function_name,
                                                step=lambda: ProviderCommonMethod.assert_equal(self, True,
                                                                                               Check220.actual_result(self))
                                                )

    @when('아이템 캐러셀중에 첫번째 상품 진입')
    def test_comm_service00226_aos_st_rt_step(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self, current_function_name,
                                                step=lambda: ItemCarouselPdpCheck.check_item_carousel_pdp(self)
                                                )

    @then('아이템 캐러셀의 상품상세 진입확인')
    def test_comm_service00226_aos_st_rt_check(self):
        current_function_name = ProviderFunctionName().get_current_function_name()
        ExceptionHandler.aos_exceptions_handler(self,
                                                current_function_name,
                                                step=lambda: ProviderCommonMethod.assert_equal(self,
                                                                                               True,
                                                                                               ItemCarouselPdpCheck.actual_result(
                                                                                                   self)),
                                                back_flow=lambda: ItemCarouselPdpCheck.back_flow(self)
                                                )
