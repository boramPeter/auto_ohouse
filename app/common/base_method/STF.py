...
paths = ["/usr/local/bin/npm", "/opt/homebrew/bin/npm",f"/Users/{user_name}/.nvm/versions/node/v20.17.0/bin/npm"]
npm_path = None
...
adb_command = os.popen("which adb").read().strip() or f"/Users/{user_name}/Library/Android/sdk/platform-tools/adb"
...
class STFManager:
    ...
    def _send_request(self, method, url, func_name, headers=None, data=None):
        ...
        raise Exception(f"{func_name} 실행시에 {url}로 보내는 데 {self.MAX_RETRIES}번 재시도하였으나 실패함.응답 코드: {response.status_code}")
